...
(declare (module-bootstrap rep.io.file-handlers))
...
;;; ::autoload-start::
(set! *file-handler-alist* (cons '("^/(([a-zA-Z0-9._-]+)@)?([a-zA-Z0-9._-]+):" . remote-file-handler) *file-handler-alist*))
...
(set! *file-handler-alist* (cons '("#tar\\b" . tar-file-handler) *file-handler-alist*))
