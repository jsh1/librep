...
(define-module rep.util.date
  ...
  (define-module-alias date rep.util.date)
...
;; Date parsing
  ...
  (defun parse-date (string #!optional (point 0))
...
character in the string. This will parse dates in RFC-822 mail messages."
	...
	(cond
	 ...
	 ((string-looking-at
	  ...
	  ;; Time spec.
	  (set! point (match-end))
	  (set! hour (string->number (substring string (match-start 1)
						(match-end 1))))
	  (set! minute (string->number (substring string (match-start 2)
						  (match-end 2))))
	  (set! second (if (equal? (match-start 3) (match-end 3))
			   0
			 (string->number (substring string (1+ (match-start 3))
						    (match-end 3)))))
	  (set! timezone (if (equal? (match-start 4) (match-end 4))
			     "UT"
			   (substring string (match-start 4) (match-end 4))))
	  (if (set! tem (assoc timezone date-timezone-alist))
	      (set! timezone (cdr tem))
	    ;; Try +-HHMM
	    (if (string-looking-at "[+-]([0-9][0-9])([0-9][0-9])" timezone)
		(set! timezone (* (if (= (string-ref timezone 0) #\+) 1 -1)
				  (+ (* 60 (string->number
					    (substring timezone
						       (match-start 1)
						       (match-end 1))))
	 ...
	 ((string-looking-at
	   ...
	   string point t)
	  ;; Found day spec
	  (set! day-abbrev (substring string (match-start 1) (match-end 1)))
	  (set! point (match-end)))
	 ...
	 ((string-looking-at
	   ...
	   string point t)
	  ;; Month name
	  (set! month-abbrev (substring string (match-start 1) (match-end 1)))
	  (set! month (cdr (assoc month-abbrev date-month-alist)))
	  (set! point (match-end)))
	 ...
	 ((string-looking-at
	  ...
	  (set! year (string->number (expand-last-match "\\1")))
	  (set! month (string->number (expand-last-match "\\2")))
	  (set! month-abbrev (car (rassq month date-month-alist)))
	  (set! day (string->number (expand-last-match "\\3")))
	  ;; XXX day of week calculation
	  (set! point (match-end)))
	 
	 (t
	  ;; Garbage in -- garbage out
	  (set! point (string-length string)))))
      
      (when (< year 0)
	(set! year (string->number (current-time-string nil "%Y"))))
      
      (when (< month 0)
	(set! month (string->number (current-time-string nil "%m"))))
      
      (when (< day 0)
	(set! day (string->number (current-time-string nil "%d"))))
	...
	(when (>= 0 g-month)
	  ...
	  (set! g-month (+ g-month 12))
	  (set! g-year (1- g-year)))
	;; (DAYS . SECONDS)
	(set! total-days (+ (- (quotient g-year 4)
			       (quotient g-year 100))
			    (quotient g-year 400)
			    ...
			    (* g-year 365)
			    -719499))
	(set! total-seconds (+ second (* 60 (+ minute
					       (- timezone)
					       (* 60 hour)))))
	(while (< total-seconds 0)
	  (set! total-seconds (+ total-seconds 86400))
	  (set! total-days (1- total-days)))
