...
;;; Functions which compile non-standard functions (ie special-forms)
  ...
  (defun get-form-opcode (form)
    ;; if no rep-compile-opcode property, assume it's the function name
    (cond ((symbol? form)
	   (or (get form 'rep-compile-opcode) form))
	  ;; must be a module-ref
	  ((eq? (car form) 'module-ref)
	   (or (get (caddr form) 'rep-compile-opcode) (caddr form)))
...
;;; speed
  ...
  (progn
    ...
    (put 'not 'rep-compile-opcode 'null?)
    ...
    (put 'variable-bound? 'rep-compile-opcode 'bound?)
