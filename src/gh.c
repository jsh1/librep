...
void
gh_enter(int argc, char *argv[], void (*c_main_prog)(int, char **))
{
  UNIMP;
}
...
repv
gh_catch(repv tag, scm_t_catch_body body, void *body_data,
...
repv
gh_standard_handler(void *data, repv tag, repv throw_args)
{
  UNIMP_RET;
}
...
repv
gh_new_procedure(const char *proc_name, repv (*fn)(),
...
repv
gh_new_procedure0_0(const char *proc_name, repv (*fn)(void))
{
  return gh_new_procedure(proc_name, fn, 0, 0, 0);
}

repv
gh_new_procedure0_1(const char *proc_name, repv (*fn)(repv))
{
  return gh_new_procedure(proc_name, fn, 0, 1, 0);
}

repv
gh_new_procedure0_2(const char *proc_name, repv (*fn)(repv, repv))
{
  return gh_new_procedure(proc_name, fn, 0, 2, 0);
}

repv
gh_new_procedure1_0(const char *proc_name, repv (*fn)(repv))
{
  return gh_new_procedure(proc_name, fn, 1, 0, 0);
}

repv
gh_new_procedure1_1(const char *proc_name, repv (*fn)(repv, repv))
{
  return gh_new_procedure(proc_name, fn, 1, 1, 0);
}

repv
gh_new_procedure1_2(const char *proc_name, repv (*fn)(repv, repv, repv))
{
  return gh_new_procedure(proc_name, fn, 1, 2, 0);
}

repv
gh_new_procedure2_0(const char *proc_name, repv (*fn)(repv, repv))
{
  return gh_new_procedure(proc_name, fn, 2, 0, 0);
}

repv
gh_new_procedure2_1(const char *proc_name, repv (*fn)(repv, repv, repv))
{
  return gh_new_procedure(proc_name, fn, 2, 1, 0);
}

repv
gh_new_procedure2_2(const char *proc_name, repv (*fn)(repv, repv, repv, repv))
{
  return gh_new_procedure(proc_name, fn, 2, 2, 0);
}

repv
gh_new_procedure3_0(const char *proc_name, repv (*fn)(repv, repv, repv))
{
  return gh_new_procedure(proc_name, fn, 3, 0, 0);
}

repv
gh_new_procedure4_0(const char *proc_name, repv (*fn)(repv, repv, repv, repv))
{
  return gh_new_procedure(proc_name, fn, 4, 0, 0);
}

repv
gh_new_procedure5_0(const char *proc_name,
		    repv (*fn)(repv, repv, repv, repv, repv))
{
...
{
  return rep_string_copy_n(s, len);
}

repv
gh_str02scm(const char *s)
{
  return rep_string_copy(s);
}

void
gh_set_substr(char *src, repv dst, long start, size_t len)
{
  UNIMP;
}
...
{
  return Fintern(rep_string_copy(symbol_str), rep_nil);
}

repv
gh_ints2scm(const int *d, long n)
{
  repv vec = rep_make_vector(n);
  for (long i = 0; i < n; i++) {
...
unsigned long
gh_scm2ulong(repv obj)
{
  return rep_get_long_uint(obj);
}

long
gh_scm2long(repv obj)
{
  return rep_get_long_int(obj);
}

char
gh_scm2char(repv obj)
{
  return rep_INTP(obj) && rep_INT(obj);
}

double
gh_scm2double(repv obj)
{
  return rep_get_float(obj);
}

char *
gh_scm2newstr(repv str, size_t *lenp)
{
  if (!rep_STRINGP(str)) {
    return NULL;
  }

  size_t len = rep_STRING_LEN(str);
...
void
gh_get_substr(repv src, char *dst, long start, size_t len)
{
...
char *
gh_symbol2newstr(repv sym, size_t *lenp)
{
  if (!rep_SYMBOLP(sym)) {
    return NULL;
  }
...
char *
gh_scm2chars(repv vector, char *result)
{
...
short *
gh_scm2shorts(repv vector, short *result)
{
  int len = gh_length(vector), i;
...
long *
gh_scm2longs(repv vector, long *result)
{
  int len = gh_length(vector), i;
...
float *
gh_scm2floats(repv vector, float *result)
{
  int len = gh_length(vector), i;
...
double *
gh_scm2doubles(repv vector, double *result)
{
  int len = gh_length(vector), i;
...
int
gh_boolean_p(repv val)
{
  return Qt;
}

int
gh_symbol_p(repv val)
{
...
int
gh_number_p(repv val)
{
  return rep_NUMERICP(val);
}

int
gh_string_p(repv val)
{
  return rep_STRINGP(val);
}

int
gh_procedure_p(repv val)
{
  val = Ffunctionp(val);
...
int
gh_eq_p(repv x, repv y)
{
  return x == y;
}

int
gh_eqv_p(repv x, repv y)
{
  repv val = Feql(x, y);
  return val && val != rep_nil;
}
...
int
gh_string_equal_p(repv s1, repv s2)
{
...
repv
gh_define(const char *name, repv val)
{
  UNIMP_RET;
}
...
repv
gh_make_string(repv k, repv chr)
{
  return Fmake_string(k, chr);
}

repv
gh_string_length(repv str)
{
  return Flength(str);
}

repv
gh_string_ref(repv str, repv k)
{
  return Faref(str, k);
}

repv
gh_string_set_x(repv str, repv k, repv chr)
{
  return Faset(str, k, chr);
}

repv
gh_substring(repv str, repv start, repv end)
{
...
repv
gh_make_vector(repv length, repv val)
{
  return Fmake_vector(length, val);
}

repv
gh_vector_set_x(repv vec, repv pos, repv val)
{
  return Faset(vec, pos, val);
}

repv
gh_vector_ref(repv vec, repv pos)
{
  return Faref(vec, pos);
}

unsigned long
gh_vector_length(repv v)
{
  return gh_length(v);
}

unsigned long
gh_uniform_vector_length(repv v)
{
  UNIMP;
  return 0;
}
...
repv
gh_uniform_vector_ref(repv v, repv ilist)
{
  UNIMP_RET;
}
...
repv
gh_module_lookup(repv module, const char *sname)
{
  UNIMP_RET;
}

repv
gh_cons(repv x, repv y)
{
  return Fcons(x, y);
}
...
repv
gh_append3(repv l1, repv l2, repv l3)
{
  repv v[3];
  v[0] = l1;
  v[1] = l2;
...
repv
gh_append4(repv l1, repv l2, repv l3, repv l4)
{
  repv v[4];
  v[0] = l1;
  v[1] = l2;
...
repv
gh_memq(repv x, repv ls)
{
  return Fmemq(x, ls);
}

repv
gh_memv(repv x, repv ls)
{
  return Fmemql(x, ls);
}

repv
gh_member(repv x, repv ls)
{
  return Fmember(x, ls);
}

repv
gh_assq(repv x, repv alist)
{
  return Fassq(x, alist);
}

repv
gh_assv(repv x, repv alist)
{
  UNIMP_RET;
}

repv
gh_assoc(repv x, repv alist)
{
...
repv
gh_call1(repv proc, repv arg)
{
  return rep_call_lisp1(proc, arg);
}

repv
gh_call2(repv proc, repv arg1, repv arg2)
{
  return rep_call_lisp2(proc, arg1, arg2);
}

repv
gh_call3(repv proc, repv arg1, repv arg2, repv arg3)
{
