...
rep_db_vprintf(void *_db, char *fmt, va_list args)
{
  ...
  if (length > db->size - db->ptr) {
    ssize_t before = db->size - db->ptr;
    ssize_t after = MIN(length - before, db->size - before);
...
#define BT_DEPTH 8
  ...
  /* It seems that in Linux/egcs-1.1 __builtin_return_address() will
     segfault when reaching the top of the stack frame. The work-around
     is to see if we can get the frame address, if so it should be safe
     to go for the return address. */
...
# define STACK_PROBE(i)				\
  ...
  for (int i = BT_BASE; i < BT_BASE+BT_DEPTH && stack[i] != 0; i++) {
...
{
  for (struct debug_buf *db = db_chain; db; db = db->next) {
    rep_db_spew(db);
  }
}
